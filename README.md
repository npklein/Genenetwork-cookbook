...
3. [GeneNetwork matrix creation](#geneNetwork-matrix-creation)<br>
 	3.1 [Slurm](#slurm)<br>
 	3.2 [Matrix creation steps overview](#matrix-creation-steps-overview)<br>
 	3.2 [Command line options](#command-line-options)<br>
4. [Docker](#docker)
5. [Ansible](#ansible)
...
The GeneNetwork matrices can be generated by running [https://github.com/npklein/brain_eQTL/tree/master/GeneNetwork](https://github.com/npklein/brain_eQTL/tree/master/GeneNetwork/step2-GeneNetworkMatrix-creation.sh). This takes as input the expression table and file of samples to use, and calculates co-reguation scores and different functional enrichments. The output is a set of matrices that are used as input for the backend of [metabrain.nl](metabrain.nl)/[genenetwork.nl](genenetwork.nl). See [example_network/step2-GeneNetworkMatric-creation.sh](step2-GeneNetworkMatric-creation.sh) for an example of how to run `step2-GeneNetworkMatrix-creation.sh`.

### Slurm

[example_network/step2-GeneNetworkMatric-creation.sh](step2-GeneNetworkMatric-creation.sh) runs [brain_eQTL/GeneNetwork/step2-GeneNetworkMatrix-creation.sh](https://github.com/npklein/brain_eQTL/tree/master/GeneNetwork/step2-GeneNetworkMatrix-creation.sh). Some of the steps in this script do some heavy computing to make correlation matrices and do PCA. Where necesarry these are submitted as jobs to a slurm scheduler. If you are not running on a cluster with a slurm scheduler, you will have to change `brain_eQTL/GeneNetwork/scripts_per_step/6_CorrelationMatrix.sh`.

### PCA++
To do the eigenvalue decomposition of the gene-gene correlation method [step 7](#matrix-creation-steps-overview) we use PCA++, as this can deal with very large correlation matrices. This is compiled on a local Mac server (iBin). When compiled on CentOS release 6.10 it gave different results for later eigenvectors and eigenvalues, we suspect due to difference in precision.

Copy the script `output/7_PCA_on_correlation_matrix/7_PCA_on_correlation.sh` that gets written by the pipeline + the file that gets used as input in that script (in the example this is `output/6_correlation_matrix/MetaBrain.deseqNorm.covarCorrected.correlation.txt`) to the iBin. Change the file paths in `7_PCA_on_correlation.sh` to relevant iBin paths. Run the script, then copy the output back to the original output location.
...
7. PCA on correlation matrix (see [PCA++](#pca++))
8. center-scales the eigenvectors
9. Correlates the eigenvectors
10. Calculates the GenNetwork predictions
11. Bonferonni corrects the prediction p-values
12. Converts matrices to the output expected by the [metabrain.nl](metabrain.nl)/[genenetwork.nl](genenetwork.nl) website. 
...
| -r  | --cronbach_cutoff | Cronbach alpha (consistensy measurement) to select number of eigenvectors to use for calculating co-regulation network. Might have to play around with this a bit. |
